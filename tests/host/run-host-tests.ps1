# FW-100 host tests — compile and run the SHIPPED C module on this PC.
#
...
$out = Join-Path $env:TEMP 'fw100_host_tests.exe'
...
if ($host_cc) {
    ...
    if ($built -ne 0) { throw 'FW-100 host harness failed to build' }
    $code = Invoke-Native $out @()
    Remove-Item $out -ErrorAction SilentlyContinue
    if ($code -ne 0) { throw "FW-100 host tests FAILED" }
    Write-Host 'FW-100 host tests: PASS'
    exit 0
}

$cross = Find-CrossCompiler
if (-not $cross) {
    Write-Warning 'No C compiler found at all. FW-100 host tests were NOT run.'
    exit 2
}

Write-Warning 'No host C compiler on this machine — the FW-100 host tests were NOT RUN.'
Write-Host "Falling back to a cross compile + link with $cross (syntax and linkage only)."
$elf = Join-Path $env:TEMP 'fw100_host_tests.elf'
...
if ($built -ne 0) { throw 'FW-100 host harness does not compile' }
Remove-Item $elf -ErrorAction SilentlyContinue
Write-Host 'FW-100 host harness: COMPILES AND LINKS (behaviour NOT verified — SKIPPED).'
