...
| **Active condition** | Re-checked every 4 kHz tick: `pedaling_active`, the ride latch, motion, hard cut, crank reverse, sensor validity, level and bank. **The pedal load is NOT re-checked.** Once triggered, easing off does not shorten the boost — only the timer or a cancel ends it. That is deliberate: a pedal stroke has dead spots, and re-testing the load would make the boost stutter at exactly the cadence it exists to help. Current is fixed at the trigger instant as `((iq_limit × (peak − trigger)) / (full_scale − trigger)) × strength%`, clamped to the level's own ceiling (max motor current and max motor power) and then to every shared limit. |
| **Stop condition** | Whichever comes first: the timer expires (`COMPLETED`), `pedaling_active` goes false (`PEDALING_STOPPED`), the ride latch drops (`PEDALING_STOPPED`), hard cut (`SAFETY_CUT`), reverse, sensor invalid, walk, calibration, level 0, level/bank change, bank rewrite, or motion lost. All act in the same control tick. Note what "stop" means: the BOOST stops immediately, and the motor current then follows whatever ramp that path uses — the level's `release_ms` for a PAS stop, `RIDE_HARD_CUT_RAMP_MS` for a hard cut. |
...
| **Re-arming** | One push gives one boost. A boost that reached ACTIVE blocks re-arming until the load falls `EXT_BOOST_RELEASE_HYST_CENTIKG` (0.5 kg) below the trigger — **whether it ran its full time or was cut short**. The case that requires the "cut short" half: a rider who stops the cranks without releasing the pedal has their boost cancelled by PAS STOP, but the push itself never ended, so on resuming the cranks the same unbroken press would otherwise confirm a fresh window 30 ms later and pay out a second boost. |
