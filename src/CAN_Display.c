	...
	switch (Ext_ID_Rx.command){
		...
		case 0x6029: //FW-015/017: read ride diagnostics v2 (peak-hold + current) (Canable only)
			if(Ext_ID_Rx.operation==1 && Ext_ID_Rx.source==5){
				...
				int32_t cur_iqr = (ride_control_get_engine()==RIDE_ENGINE_TSDZ) ?
				...
				uint8_t dg[32];
				dg[0]=0x44; dg[1]=0x47; dg[2]=2; //'D''G' ver2 (32 B)
				...
				dg[24]=rin->torque_assist_filtered&0xFF; dg[25]=(rin->torque_assist_filtered>>8)&0xFF; //current fast pressure
				dg[26]=cur_iqr&0xFF; dg[27]=(cur_iqr>>8)&0xFF;                     //current iq request before final ramp
				dg[28]=cur_iqs&0xFF; dg[29]=(cur_iqs>>8)&0xFF;                     //current iq_setpoint
				uint16_t c=0xFFFF; for(uint8_t i=0;i<30;i++){c^=(uint16_t)dg[i]<<8; for(uint8_t b=0;b<8;b++)c=(c&0x8000)?((c<<1)^0x1021):(c<<1);}
				dg[30]=c&0xFF; dg[31]=(c>>8)&0xFF;
				send_multiframe(Ext_ID_Rx.command, (char*)&dg[0], 32);
		...
		case 0x6017: //FW-022: exact Hall calibration telemetry (Canable/BESST only)
			if(Ext_ID_Rx.operation==1 && Ext_ID_Rx.source==5){
				uint8_t hall[36];
				hall[0]='H'; hall[1]='C'; hall[2]=1; hall[3]=MS->hall_angle_detect_flag;
